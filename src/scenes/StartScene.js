...
import dishUrl from "../assets/mapo-dish-placeholder.svg?url";
...
export class StartScene {
  ...
  async _loadDish() {
    try {
      ...
      sprite.width = LAYOUT.dish.w;
      sprite.height = LAYOUT.dish.h;
      this.dishSprite = sprite;
      this.dish.addChild(sprite);
      // re-layout once the asset arrives in case our screen size already known
      if (this._lastResize) this.resize(...this._lastResize);
    } catch (e) {
      console.warn("StartScene: failed to load mapo dish placeholder", e);
    }
  }
  ...
  resize(screenW, screenH) {
    ...
    y += LAYOUT.dish.h + LAYOUT.gapDishButton;
